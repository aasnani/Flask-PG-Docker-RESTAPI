...
def list_books():
    try:
        ...
        logger.error(msg=f"GET /book failed with message: {str(e)}", exc_info=True)
        response = BaseResponse(message=f"There was a problem getting all the book objects: {str(e)}")
...
def create_books():
    try:
        ...
        logger.error(msg=f"POST /book failed with message: {str(e)}", exc_info=True)
        response = BaseResponse(message=f"There was a problem creating the book object: {str(e)}")
        return jsonify(response.toDict()), 500
...
def get_book_by_id(book_id):
    try:
        ...
        logger.error(msg=f"GET /book/{book_id} failed with message: {str(e)}", exc_info=True)
        response = BaseResponse(message=f"There was a problem getting the book object with id={book_id}: {str(e)}")
        return jsonify(response.toDict()), 500
...
def delete_book_by_id(book_id):
    try:
        ...
        logger.error(msg=f"DELETE /book/{book_id} failed with message: {str(e)}", exc_info=True)
        response = BaseResponse(message=f"There was a problem deleting the book object with id={book_id}: {str(e)}")
        return jsonify(response.toDict()), 500
...
def get_books_of_author(author_id):
    try:
        ...
        logger.error(msg=f"GET /author/{author_id}/books failed with message: {str(e)}", exc_info=True)
        response = BaseResponse(message=f"There was a problem getting the book objects of Authors with id={author_id}: {str(e)}")
        return jsonify(response.toDict()), 500